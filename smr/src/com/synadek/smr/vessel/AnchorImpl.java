...
public class AnchorImpl extends VesselComponentImpl implements Anchor, ControlledActuator {
  ...
  /**
   * Anchor configuration parameter key.
   */
  ...
  /**
   * drawAnchor is the utility method that moves the anchor to a desired depth
   * by driving the anchor motor. Note: depth really only indicates the length
   * of anchor chain extended from the vessel. In shallow water, the anchor may
   * not achieve this vertical depth in the water and the vessel may drift
   * horizontally to the extent of the chain.
   *
   * @param desiredDepth
   *          is the new depth to which the anchor should be set. Depth of zero
   *          means anchor fully raised.
   */
  ...
  public void setOutputValue(float newValue) {
    ...
    log.debug("Stepping anchor motor {} steps to move anchor chain {} meters.",
        Float.valueOf(stepsRequired), Float.valueOf(metersToMove));
  ...
  public void resetConfiguration() {
    ...
    final JSONObject depthProp = JsonSchema.schemaNumber(
        "Maximum depth of anchor i.e., length of anchor chain in meters.", 0.0, true);
    final JSONObject speedProp = JsonSchema
        .schemaNumber("Maximum speed of anchor movement in meters per second.", 0.0, true);
