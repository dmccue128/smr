 ...
 */
public class VesselPhidget22 extends AbstractVesselModel implements DigitalInputStateChangeListener,
		VoltageInputSensorChangeListener, VoltageInputVoltageChangeListener {

	/**
	 * Map Physical Device Model device ids to Phidget meta data.
	 */
	private final static Map<PhysicalDeviceType, VesselPhidgetMeta> GpioMap = createPhysicaltoGpioMap();

	/**
	 * Reverse map from Phidget physical digital input indexes (0-7) to PDM IDs.
	 */
	private final static Map<Integer, PhysicalDeviceType> digitalInputMap = new HashMap<>();

	/**
	 * Reverse map from Phidget physical digital output indexes (0-7) to PDM IDs.
	 */
	private final static Map<Integer, PhysicalDeviceType> digitalOutputMap = new HashMap<>();
	/**
	 * Reverse map from Phidget physical analog input indexes (0-7) to PDM IDs.
	 */
	private final static Map<Integer, PhysicalDeviceType> analogInputMap = new HashMap<>();
	/**
	 * Channel map for digital inputs.
	 */
	private final static Map<Integer, DigitalInput> digitalInputChannelMap = new HashMap<>();
	/**
	 * Channel map for digital outputs.
	 */
	private final static Map<Integer, DigitalOutput> digitalOutputChannelMap = new HashMap<>();
	/**
	 * Channel map for analog inputs.
	 */
	private final static Map<Integer, VoltageInput> analogInputChannelMap = new HashMap<>();
	...
	private static Map<PhysicalDeviceType, VesselPhidgetMeta> createPhysicaltoGpioMap() {
		...
		myMap.put(PhysicalDeviceType.PHY_BATTERY_VOLTAGE_SENSOR,
				new VesselPhidgetMeta("battery voltage sensor", VesselPhysicalModel.PinType.ANALOG_INPUT_TYPE, 1));
	 ...
	 * 
	 * @param dev the physical analog input device to read
	 * @return the value
	...
	private final double getAiValue(final PhysicalDeviceType dev) throws ComponentException {
		...
		try {
			...
			if (pin.getType() != PinType.ANALOG_INPUT_TYPE) {
				throw new ComponentException("Attempt to get analog input state for a pin of type " + pin.getType());
			}
	...
	public double getAnalogChangeTrigger(final PhysicalDeviceType dev) throws ComponentException {
		...
		try {
			...
			// Validate input parameter
			if (pin.getType() != PinType.ANALOG_INPUT_TYPE) {
				throw new ComponentException("Attempt to get change trigger value for a pin of type " + pin.getType());
			}
	...
	public int getAnalogDataRate(final PhysicalDeviceType dev) throws ComponentException {
		...
		try {
			...
			// Validate input parameter
			if (pin.getType() != PinType.ANALOG_INPUT_TYPE) {
				throw new ComponentException("Attempt to get data rate for a pin of type " + pin.getType());
			}
	...
	public int getAnalogDataRateMax(final PhysicalDeviceType dev) throws ComponentException {
		...
		try {
			...
			// Validate input parameter
			if (pin.getType() != PinType.ANALOG_INPUT_TYPE) {
				throw new ComponentException("Attempt to get data rate maximum for a pin of type " + pin.getType());
			}
	...
	public int getAnalogDataRateMin(final PhysicalDeviceType dev) throws ComponentException {
		...
		try {
			...
			// Validate input parameter
			if (pin.getType() != PinType.ANALOG_INPUT_TYPE) {
				throw new ComponentException("Attempt to get data rate minimum for a pin of type " + pin.getType());
			}
	 ...
	 * @return the value
	 * @throws ComponentException if underlying device is not connected or pin is
	 *                            not mapped
	 */
	 ...
	 * 
	 * @param dev the physical digital input device to read
	 * @return the value.
	 * @throws ComponentException if underlying device is not connected
	 */
	@Override
	public final boolean getDigitalInputState(final PhysicalDeviceType dev) throws ComponentException {
	...
	public final boolean getDiState(final PhysicalDeviceType dev) throws ComponentException {
		...
		try {
			...
			if (pin.getType() != PinType.DIGITAL_INPUT_TYPE) {
				throw new ComponentException("Attempt to get digital input state for a pin of type " + pin.getType());
			}
	 ...
	 * 
	 * @param dev the device for which the value is sought
	 * @return the state
	 */
	private boolean getDoState(final PhysicalDeviceType dev) throws ComponentException {
		...
		try {
			...
			if (pin.getType() != PinType.DIGITAL_OUTPUT_TYPE) {
				throw new ComponentException("Attempt to get digital output state for a pin of type " + pin.getType());
			}
	...
	public int getPinNumber(final PhysicalDeviceType dev) {
		...
		// Map the VesselPhysicalModel device to a descriptor for a physical pin on the
		// Phidget
		final VesselPhidgetMeta pin = GpioMap.get(dev);
		if (pin != null) {
			return pin.getIndex();
	 ...
	 * 
	 * @see
	 * com.phidget22.VoltageInputSensorChangeListener#onSensorChange(com.phidget22.
	 * VoltageInputSensorChangeEvent)
	 */
	@Override
	public void onSensorChange(VoltageInputSensorChangeEvent arg0) {
		...
		// Map the Phidget pin number to a physical device in the VesselPhysicalModel
		final PhysicalDeviceType dev = analogInputMap.get(Integer.valueOf(physicalPinNumber));

		if (dev == null) {
			log.error("Received event on pin " + physicalPinNumber + " but no device is mapped to this pin");
			return;
		}
	 ...
	 * 
	 * @param device the Pio device id
	 * @param index  is the physical pin number on the Phidget to which this device
	 *               is connected (0-7) e.g., 1 for digital input 1
	 */
			...
			public void onAttach(AttachEvent ae) {
				...
				try {
					if (phid.getDeviceClass() != DeviceClass.VINT) {
						...
						log.debug("Attached channel " + phid.getChannel() + " on device " + phid.getDeviceSerialNumber()
		...
		ch.addDetachListener(new DetachListener() {
			...
			public void onDetach(DetachEvent de) {
				...
				try {
					if (phid.getDeviceClass() != DeviceClass.VINT) {
						log.info(
								"Detached channel " + phid.getChannel() + " on device " + phid.getDeviceSerialNumber());
					} else {
						log.info("Detached channel " + phid.getChannel() + " on device " + phid.getDeviceSerialNumber()
								+ " hub port " + phid.getHubPort());
					}
				} catch (PhidgetException ex) {
					log.error(ex.getDescription());
	 ...
	 * @param device the Pio device id
	 * @param index  is the physical pin number on the Phidget to which this device
	 *               is connected (0-7) e.g., 1 for digital output 1
	 */
	private DigitalOutput openDigitalOutputChannel(final PhysicalDeviceType device, final int index) {
	 ...
	 * @param device the Pio device id
	 * @param index  is the physical pin number on the Phidget to which this device
	 *               is connected (0-7) e.g., 1 for analog input 1
	 */
	 ...
	 * amount that an inputs must change between successive SensorChangeEvents. This
	 * is based on the 0-1000 range provided by getSensorValue. This value is by
	 ...
	 * @param val is the voltage delta value
	 * @throws ComponentException if underlying device is not connected
	 */
	@Override
	public final void setAnalogChangeTrigger(final PhysicalDeviceType dev, final double val) throws ComponentException {
		if (val < 0 || val > 1000) {
		...
		try {
			...
			// Validate input parameter
			if (pin.getType() != PinType.ANALOG_INPUT_TYPE) {
				throw new ComponentException("Attempt to set change trigger for a pin of type " + pin.getType());
			}
	 ...
	 * 
	 * @param dev the Pio device ID
	 * @param val is the sampling interval (milliseconds)
	 * @throws ComponentException if underlying device is not connected
	 */
	public final void setAnalogDataRate(final PhysicalDeviceType dev, final int val) throws ComponentException {
		...
		try {
			...
			// Validate input parameter
			if (pin.getType() != PinType.ANALOG_INPUT_TYPE) {
				throw new ComponentException("Attempt to set data rate for a pin of type " + pin.getType());
			}
	 ...
	 */
	@Override
	public void setDigitalInputState(final PhysicalDeviceType dev, boolean newVal) throws ComponentException {
	 ...
	 * 
	 * @param idx the digital output pin index
	 ...
	 * 
	 * @param dev the physical digital output device to update
	 * @param val the boolean value to set.
	 * @throws ComponentException if underlying device is not connected
	 */
	@Override
	public final void setDigitalOutputState(final PhysicalDeviceType dev, final boolean val) throws ComponentException {
