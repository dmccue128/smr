...
public class SunlightImpl extends AbstractComponent implements Sunlight {
	...
	public float solarEnergy(Duration duration) {
		// TODO implement reporting of aggregate sunlight received over a period of time
		return 0.0f;
	}
	...
	public double getGeoMeanLongSun(double julianCentury) {
		double geoMeanLongSun = (280.46646 + julianCentury * (36000.76983 + julianCentury * 0.0003032)) % 360.0;
	...
	public double getSunEqOfCtr(double julianCentury) {
	...
	public double getSunAppLong(double julianCentury) {
	...
	public double getVarY(double julianCentury) {
	...
	public double getEqOfTime(double julianCentury) {
		...
		double eqOfTime = 4.0 * Math.toDegrees(varY * Math.sin(2 * geoMeanLongSunRadians)
				...
				+ 4 * eccentEarthOrbit * varY * Math.sin(geoMeanLongSunRadians) * Math.cos(2 * geoMeanLongSunRadians)
				- 0.5 * varY * varY * Math.sin(4 * geoMeanLongSunRadians)
				- 1.25 * eccentEarthOrbit * eccentEarthOrbit * Math.sin(2 * geoMeanAnomSunRadians));
		return eqOfTime;
	}

	public double getHaSunrise(double lat, double julianCentury) {
		double sunDeclinRadians = Math.toRadians(getSunDeclin(julianCentury));
		double haSunrise = Math.toDegrees(Math
				.acos(Math.cos(Math.toRadians(90.833)) / (Math.cos(Math.toRadians(lat)) * Math.cos(sunDeclinRadians))
	...
	public double getTrueSolarTime(double lon, double zone, double pctOfDay, double julianCentury) {
		double minutesPastMidnight = pctOfDay * MINUTES_PER_DAY;
		double trueSolarTimeSubtotal = (minutesPastMidnight + getEqOfTime(julianCentury) + (4.0 * lon) - (60.0 * zone));
	...
	public double getSunDeclin(double julianCentury) {
		double sunDeclin = Math.toDegrees(Math.asin(Math.sin(Math.toRadians(getObliqueCorr(julianCentury)))
	 ...
	 */
	public double getSolarZenithAngle(double lon, double lat, double zone, double pctOfDay, double julianCentury) {
		double sunDeclinRadians = Math.toRadians(getSunDeclin(julianCentury));
		double latRadians = Math.toRadians(lat);
		double hourAngleRadians = Math.toRadians(getHourAngle(lon, zone, pctOfDay, julianCentury));
		double solarZenithAngle = Math.toDegrees(Math.acos(Math.sin(latRadians) * Math.sin(sunDeclinRadians)
	 ...
	 * @return the solar zenith angle
	 */
	public double getSolarAzimuthAngle(double lon, double lat, double zone, double pctOfDay, double julianCentury) {

		double solarAzimuthAngle;

		if (getHourAngle(lon, zone, pctOfDay, julianCentury) > 0) {
			solarAzimuthAngle = (Math
					.toDegrees(Math.acos(((Math.sin(Math.toRadians(lat))
			...
			solarAzimuthAngle = (540.0 - Math.toDegrees(Math.acos(((Math.sin(Math.toRadians(lat))
					* Math.cos(Math.toRadians(getSolarZenithAngle(lon, lat, zone, pctOfDay, julianCentury))))
					...
					/ (Math.cos(Math.toRadians(lat))
							* Math.sin(Math.toRadians(getSolarZenithAngle(lon, lat, zone, pctOfDay, julianCentury)))))))
					% 360;
		}
