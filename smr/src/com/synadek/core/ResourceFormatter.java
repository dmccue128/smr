...
public class ResourceFormatter {
   ...
   *
   * @param args
   *          is the array of arguments to apply to the pattern
   * @param resourceKey
   *          is the key to find the pattern to use for the formatting
   * @param locale
   *          is the locale for which the resources are sought
   * @return the localized message
   */
  public static String formatMessage(final Object[] args, final String resourceKey,
      final Locale locale) {
    String result;
    ...
    try {
      ...
      if (result == null) {
        log.error("Formatting of resourceKey, {}, with pattern, {}, returned null", resourceKey,
            pattern);
        result = RESOURCE_ERROR;
      }
    } catch (NullPointerException n) {
      log.error("Null pointer exception formatting message: {}",
          formatDebugMsg(args, resourceKey, pattern));
      result = RESOURCE_ERROR;
    } catch (IllegalArgumentException e) {
      log.error("Illegal message format request using pattern, {}: {}", pattern, e.getMessage());
      // pattern should always be valid -- print a stack trace to assist
      // debugging
      e.printStackTrace();
      result = RESOURCE_ERROR;
    }
  ...
  public static String getMessage(final String resourceKey, final Locale locale) {
    ...
    try {
      ...
      log.error("Non-string result from look up of resource key, {}: {}", resourceKey,
          c.getMessage());
   ...
   *          is the key to find the pattern to use for the formatting
   * @param pattern
   *          is the retrieved message pattern from the resource file
   * @return a debug message string displaying information about the error
   */
  private static String formatDebugMsg(final Object[] args, final String resourceKey,
  ...
  /**
   * getSessionResources locates the most appropriate resource bundle in the
   * pool to use for localizing messages for the user interface.
   *
   * @param locale
   *          is the locale for which the resources are sought
