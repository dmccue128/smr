...
/**
 * Abstract class implementing core methods common to all Components.
 *
 * @formatter:off
 * 
 *                Implementation of the abstract class, Component, includes
 *                three functions of a component: 1) demographic info about the
 *                component (e.g., component name) 2) component health status 3)
 *                component configuration parameters
 * 
 *                This class also provides translation services for components
 *                to format user messages.
 * 
 * @formatter:on
 */
public abstract class AbstractComponent implements Component {
   ...
   */
  private static final Map<Status, String> statusMap = Map.of(Status.ACTIVE, "status.active",
      Status.IMPAIRED, "status.impaired", Status.INACTIVE, "status.inactive");
   ...
   *
   * @param name
   *          is a unique name for this component instance
   */
   ...
   *         resource not found
   */
  protected final String formatUserMessage(final Object[] args, final String resourceKey,
      final Locale locale) {
   ...
   *
   * @param bundleName
   *          is the base name of the resource bundle to use
   * @param args
   *          is the array of arguments to apply to the pattern
   * @param resourceKey
   *          is the key to find the pattern to use for the formatting
   ...
   *         resource not found
   */
  protected final String formatUserMessage(final String bundleName, final Object[] args,
  ...
  /**
   * getResourceBundle locates the most appropriate resource bundle for
   * localizing messages for the user interface.
   *
   * @param basename
   *          is the base name of the resource file to use. If null, a base name
   *          will be generated by appending RESOURCES_BUNDLE_EXTENSION to the
   *          package name.
   * @param locale
   *          is the locale for which the resources are sought or null for the
   *          default locale
   * @return a resource bundle
   */
  private ResourceBundle getBundle(final String basename, final Locale locale) {

    // Bundle name is generated from package name concatenated with
    // ".Resources"
    if (basename == null) {
   ...
   *
   * @param locale
   *          locale in which to present information
  ...
  public String getStatusMessage(Locale locale) {
    ...
    if (status != null && !status.isEmpty()) {
      ...
      String sep = "";
      for (Object keyObj : status.keySet()) {
    ...
    // If the status object contained no elements, use localized label for
    // status
    // code
    if (generatedMessage == null || generatedMessage.length() == 0) {
      // Get resource key for the user message corresponding to object's
      // status
  ...
  /**
   * Return a user message based on preferred locale using default resource
   * bundle for the class.
   *
   * @param resourceKey
   *          is the key to find the pattern to use for the formatting
   * @param locale
   ...
   *         if resource not found
   */
  protected final String userMessage(final String bundleName, final String resourceKey,
      final Locale locale) {
    ...
    try {
      ...
      // misspelled
      log.error(
          "Failed attempt to look up user resource key, " + resourceKey + ": " + mre.getMessage());
      // In default, use the untranslated resource key as the message
      result = RESOURCE_ERROR;
    } catch (ClassCastException cce) {
      log.error("Non-string result from look up of user resource key, " + resourceKey + ": "
          + cce.getMessage());
