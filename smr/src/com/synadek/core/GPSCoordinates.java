...
public class GPSCoordinates {
	...
	private static final String[] compassPoints = { "north", "north east", "east", "south east", "south", "south west",
			"west", "north west" };
	 ...
	 * @param lat  the latitude in decimal degrees
	 * @param lon  the longitude in decimal degrees
	 * @param elev the elevation (ref. ellipsoid)
	 */
	 ...
	 * 
	 * @param obj the JSON Object using degrees as units
	 */
	...
	/**
	 * Alternate constructor parses string representations of latitude and longitude
	 * in any of three formats: Degrees,Decimal Minutes(DDM) or
	 * Degrees,Minutes,Seconds (DMS) or Decimal Degrees (DD).
	 * 
	 * @param lat the latitude in DMS format (36�59'21.2"N) or DDM format (36
	 *            59.35333) or DD format (36.989213)
	 * @param lon the longitude in DMS format(84�13'53.3"W) or DDM format(-84
	 *            13.888333) or DD format (-84.231474)
	 * @throws NumberFormatException if the inputs cannot be parsed
	 */
	 ...
	 * @param lon  the longitude in DMS format(84�13'53.3"W) or DDM format(-84
	 *             13.888333) or DD format (-84.231474)
	 * @param elev the elevation in meters (from ellipsoid)
	 * @throws NumberFormatException if the inputs cannot be parsed
	 */
	public GPSCoordinates(final String lat, final String lon, final String elev) throws NumberFormatException {
		latitude = parseLat(lat);
		// If latitude parsed successfully, try to parse longitude
		longitude = parseLon(lon);
	 ...
	 * 
	 * @param toLocation the other GPS coordinates
	...
	/**
	 * Get GPS coordinates in Decimal Degrees format. Example: 36.989213,-84.231474
	 * 
	...
	/**
	 * Get GPS coordinates in Degrees, Decimal Minutes format. Example: 36 59.35333
	 * -84 13.888333
	 * 
	 * @return the coordinates
	 */
	...
	public int getFinalBearing(final GPSCoordinates toLocation) {
		...
		double x = Math.cos(this.getLatitude().radians()) * Math.sin(toLocation.getLatitude().radians())
	...
	/**
	 * Get latitude in Degrees, Minutes, Seconds (DMS) format. Example: 36�59'21.2"N
	 * 
	 * @return the latitude
	 */
	public final String getLatitudeDMS() {
		...
		return String.format("%g%s%g'%g\"%c", Double.valueOf(degrees), degreeSymbol, Double.valueOf(minutes),
				Double.valueOf(seconds), Character.valueOf(ns));
	}

	/**
	 * Get the longitude in Degrees, Decimal Minutes format. Example: -84 13.888333
	 * 
	...
	public final String getLongitudeDMS() {
		...
		return String.format("%g%s%g'%4.1g\"%c", Double.valueOf(degrees), degreeSymbol, Double.valueOf(minutes),
	...
	public GPSCoordinates getMidpoint(final GPSCoordinates toLocation) {
		...
		double Bx = Math.cos(toLocation.getLatitude().radians()) * Math.cos(deltaLongitude);
		double By = Math.cos(toLocation.getLatitude().radians()) * Math.sin(deltaLongitude);
		double midLatitude = Math.atan2(
				Math.sin(this.getLatitude().radians()) + Math.sin(toLocation.getLatitude().radians()),
				Math.sqrt((Math.cos(this.getLatitude().radians()) + Bx) * (Math.cos(this.getLatitude().radians()) + Bx)
						+ By * By));
		double midLongitude = this.getLongitude().radians()
				+ Math.atan2(By, Math.cos(this.getLatitude().radians()) + Bx);
